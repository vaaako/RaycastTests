...
void drawRay() {
	...
	for(r=0;r<120;r++) { //60
		...
		float shade=1;
		glColor3f(0,0.8,0); // Default Color
		if(disV<disH) { // Horizontal hit first
			hmt=vmt;
			shade=0.5; // Shading
			rx=vx; ry=vy; disH=disV;
			glColor3f(0,0.6,0); // Far color ("Light")
		}
		...
		for(y=0; y<lineH; y++) {
			...
			// DrawPoint(r*8, y+lineOff, 8, All_Textures, pixel);//getColor(All_Textures, pixel, shade));
		...
		for(y=lineOff+lineH; y<SCREEN_HEIGHT; y++) {
			...
			if(mp>0){ glPointSize(8); glColor3ub(All_Textures[pixel+0],All_Textures[pixel+1],All_Textures[pixel+2]); glBegin(GL_POINTS); glVertex2i(r*8,SCREEN_HEIGHT-y); glEnd();}
